...
No setup is required: the skill works in any Git repository, including one that has never been prepared for agentic work (no `.audit/`, no `.claude/`, no `CLAUDE.md`/`AGENTS.md`). In such a bare repository the built-in profiles are used, artifacts go to the neutral `.audit/local/audits/`, and the helper adds a local, untracked ignore rule for them (see below).
...
Resolution is explicit: `--artifact-root`, profile `artifact_root`, neutral `.audit/local/audits/` when `.audit/` exists (`neutral-default`), legacy `.claude/local/audits/` only for genuinely legacy repositories — legacy `.claude/audit/`, existing `.claude/local/audits/`, or `.claude/local/audit.overrides.yaml` present and no `.audit/` (`legacy-fallback`) — and otherwise neutral `.audit/local/audits/` (`neutral-bare-default`).

The helper refuses to write artifacts inside a git repository unless the complete audit directory itself is ignored; it also checks every planned standard artifact and unpredictable focused-verification candidates as defense in depth. `--allow-unignored-artifacts` is incompatible with immutable target snapshots and is rejected because generated reports would change the target.

Auto-exclude: when the selected root is the neutral default (`neutral-default` or `neutral-bare-default`, never an explicit `--artifact-root` or profile `artifact_root`) and the audit directory is inside a Git repository but not ignored, the helper appends an anchored `/.audit/local/` rule (prefixed with the project's path when it is a subdirectory) to that repository's local exclude file, resolved with `git rev-parse --git-path info/exclude` so linked worktrees and submodules get the correct file. The edit is idempotent, creates the file if absent, preserves existing content, refuses symbolic-link exclude paths, touches only the repository that contains the artifact directory, and happens before any snapshot or artifact is captured; the normal ignore checks then run unchanged. `audit.yml` records the outcome under `artifact_exclude`. Pass `--no-auto-exclude` to keep the exclude file untouched and fail with instructions instead. To ignore artifacts yourself, add `.audit/local/` to `.gitignore` or `.git/info/exclude`; retain `.claude/local/` ignores while legacy artifacts exist. It also rejects symbolic-link path components, invalid multi-component audit IDs, and any audit-ID collision instead of reusing or overwriting an existing directory. Startup revalidates the target after writing the initial metadata and prompts.
