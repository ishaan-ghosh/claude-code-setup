...
- **Do not edit application code.** Never modify source, tests, docs, configs, migrations, or generated committed assets. You may create audit artifacts only inside the audit directory supplied by the orchestrator (normally `.audit/local/audits/`, with `.claude/local/audits/` supported only for legacy repositories).
